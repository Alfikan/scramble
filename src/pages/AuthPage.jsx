...
const AuthPage = () => {
  ...
  const handleSocialAuth = async (provider) => {
    ...
    try {
      const result = provider === "google" ? await signInWithGoogle() : null;
      console.log("Google sign in result:", result);

      if (result && result.success) {
        navigate("/dashboard");
      } else if (!result || !result.success) {
        const errMsg = result?.error || "Social authentication failed";
        console.error("Google auth error:", errMsg);
        alert("Google Sign-In Error: " + errMsg); // Debug alert
        setError(errMsg);
      }
    } catch (err) {
      console.error("Social auth error:", err);
      alert("Error: " + err.message); // Debug alert
  ...
  const getSubtitle = () => {
    switch (mode) {
      ...
      case "signup":
        return "Create your account and start studying together";
      case "forgot":
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-muted-gray">{getSubtitle()}</p>
              </div>
              ...
              <AnimatePresence>
                {error && (
                  ...
                  >
                    <p className="text-warning-red text-sm font-medium">
                      {error}
                    </p>
                  </motion.div>
                )}
                ...
                {success && (
                  ...
                  >
                    <p className="text-success-green text-sm font-medium">
                      {success}
                    </p>
                  </motion.div>
                )}
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="relative">
                    ...
                    <div className="relative flex justify-center text-sm">
                      <span className="px-2 bg-light-cream text-muted-gray">
                        or
                      </span>
                    </div>
                  </div>
